...
import { UpdateCategory } from '@/hooks/CategoryHook';
...
const EditCategory = ({ editCatData, onSubmit }) => {
  ...
  useEffect(() => {
    if (editCatData) {
      setFormData({
        category_id: editCatData.category_id,
        user_id: editCatData.user_id,
        user_name: editCatData.user.name,
        category_name: editCatData.category_name,
        category_description: editCatData.category_description,
        category_type: editCatData.category_type,
      });
    }
  }, [editCatData]);
...
useEffect(() => {
  ...
  return (
    <form className="space-y-2" onSubmit={updateCategory}>
      ...
      <div className='pb-3'>
        ...
        <div className="mt-2 flex justify-end text-xs text-gray-600">
          <span>{`${formData.category_name.length} / ${100} characters`}</span>
        </div>
      </div>
