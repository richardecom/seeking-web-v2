...
const EditItem = ({itemData, onSubmit}) => {
  ...
  const selectMLocation = (value) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, location_id: undefined }));
        ...
        if (error instanceof z.ZodError) {
            setErrors((prev) => ({ ...prev, location_id: error.errors[0].message }));
        }
    }
  };
  ...
  const checkboxChange = (event) =>{
    ...
    setFormData((prev) => ({ ...prev, [id]: event.target.dataset.state !== 'checked' }));
  }
  ...
  useEffect(() => {
    if (itemData) {
      setFormData({
        ...
        user_id: itemData.user_id || 0, 
        user_name: itemData.user.name || '',
        location_id: itemData.location_id || 0,
        location_key: itemData.location.building || '',
        category_id: itemData.category_id || 0,
        item_name: itemData.item_name || '',
        description: itemData.description || '',
        quantity: itemData.quantity || 0,
        rating: itemData.rating || 0,
        expiry_date: itemData.expiry_date || null,
        perishable: itemData.perishable === 'Yes'? true : false,
        always_stock: itemData.always_stock === 'Yes'? true : false,
        uncountable: itemData.uncountable === 'Yes'? true : false,
        favorite: itemData.favorite === 'Yes'? true : false
      });
    }
    ...
    <form className="space-y-2" onSubmit={updateItem}>
        ...
        <div className="relative w-full pb-2">
          ...
          <label htmlFor="user_name" className="flex items-center text-sm font-medium leading-4 text-gray-900">
              ...
              {errors.user_id && (
                  <p className="text-red-500 text-xs font-normal py-1">{errors.user_id}</p>
              )}
          </label>
        ...
        <div className="relative w-full pb-2">
          <div className='w-full'>
          <label htmlFor="location_key" className="flex items-center text-sm font-medium leading-4 text-gray-900">
              <span className="pr-3 py-2">Select Location</span>
              {errors.location_id && (
                  <p className="text-red-500 text-xs font-normal py-1">{errors.location_id}</p>
              )}
          </label>
          <div className="relative">
            <input
              ...
              type="text"
              required
              disabled={formData.user_id < 0 || formData.user_id === null || formData.user_id === 0}
              value={formData.location_key}
              onChange={searchLocation}
              className="block w-full rounded-md border text-sm border-gray-300 p-2 shadow-sm transition duration-200 placeholder:text-gray-400 focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600"
              placeholder={` ${formData.user_id < 0 || formData.user_id === null || formData.user_id === 0 ? '' : 'Search User Locations'}`}
            />
            {showLocation && (
              <ul className="absolute z-10 w-full border border-gray-300 bg-white rounded-md shadow-lg max-h-48 overflow-y-auto"> {/* Set max height and overflow */}
        ...
        <div >
            ...
            <span className="pr-3 py-2">Item Name</span>
              {errors.item_name && (
                  <p className="text-red-500 text-xs font-normal  py-1">{errors.item_name}</p>
              )}
            </label>
            <div>
      ...
      <div>
        <label htmlFor="description" className="flex items-center text-sm font-medium leading-4 text-gray-900">
              <span className="pr-3 py-2">Description</span>
              {errors.description && (
                  <p className="text-red-500 text-xs font-normal  py-1 px-1">{errors.description}</p>
              )}
          </label>
        <div>
          <textarea 
        ...
        <div className='pb-2'>
          <label htmlFor="quantity" className="flex items-center text-sm font-medium leading-4 text-gray-900">
          <span className="pr-3 py-2">Quantity</span>
            {errors.quantity && (
              <p className="text-red-500 text-xs font-normal  py-1">{errors.quantity}</p>
            )}
          </label>
          <div className="mt-2">
        ...
        <div>
          ...
          <span className="pr-3 py-2">Rating</span>
            {errors.rating && (
              <p className="text-red-500 text-xs font-normal  py-1">{errors.rating}</p>
            )}
          </span>
          <ReactStars
        ...
        <Checkbox id="perishable" name='perishable' checked={formData.perishable} onClick={checkboxChange}/>
          ...
          <Checkbox id="always_stock" name='always_stock' checked={formData.always_stock} onClick={checkboxChange} />
          <div className="grid gap-1.5 leading-none pr-3">
            <label
          ...
          <Checkbox id="uncountable" name='uncountable' checked={formData.uncountable} onClick={checkboxChange}/>
          <div className="grid gap-1.5 leading-none pr-3">
            <label
              htmlFor="uncountable"
              className="text-sm mt-[2px] font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          ...
          <Checkbox id="favorite" name='favorite' checked={formData.favorite} onClick={checkboxChange}/>
          <div className="grid gap-1.5 leading-none pr-3">
            <label
              htmlFor="favorite"
              className="text-sm mt-[2px] font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {formData.expiry_date ? format(formData.expiry_date, "PPP") : <span>Expiry Date</span>}
              </Button>
            </PopoverTrigger>
