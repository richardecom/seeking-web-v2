...
import { uploadFilesToS3 } from "@/lib/upload_aws";
    ...
    if (user) {
      ...
      if (files.length > 0) {
        ...
        if (process.env.RUN_ENV === "local") {
          ...
          const uploadedFileUrls = (await uploadFilesToS3(files)) as any;
          if (uploadedFileUrls.error) {
            return NextResponse.json({
              status: 400,
              message: uploadedFileUrls.error,
            });
          }
          user.image = uploadedFileUrls[0];
        }
      }
